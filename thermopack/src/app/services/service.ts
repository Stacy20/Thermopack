...
export class MainService {
  ...
  private servicesSubject: BehaviorSubject<{
    services: Services[];
    totalCount: number;
  }> = new BehaviorSubject<{ services: Services[]; totalCount: number }>({
    services: [],
    totalCount: 0,
  });
  public services$: Observable<{ services: Services[]; totalCount: number }> =
  ...
  constructor(private http: HttpClient) {
    ...
    this.isLoggedIn =
      lastLogin && this.dayHasntPassed(lastLogin) ? this.isLoggedIn : false;
  ...
  private productsSubject: BehaviorSubject<{
    products: Products[];
    totalCount: number;
  }> = new BehaviorSubject<{ products: Products[]; totalCount: number }>({
    products: [],
    totalCount: 0,
  });
  public products$: Observable<{ products: Products[]; totalCount: number }> =
  ...
  areProducts(): Observable<boolean> {
    const url = `${this.connectionUrl}products/check/not_empty`;
  ...
  getAllCategories(): Observable<Categories[]> {
    ...
    return this.http.get<Categories[]>(url).pipe(catchError(() => of([])));
  }
  ...
  getCategoryByName(name: string): Observable<Categories> {
    ...
    return this.http
      .get<Categories>(url)
      .pipe(catchError(() => of({} as Categories)));
  }

  createCategory(name: string): Observable<Categories> {
    const url = `${this.connectionUrl}categories`;
    return this.http
      .post<Categories>(url, { name: name })
      .pipe(catchError(() => of({} as Categories)));
  }
  ...
  updateCategoryByName(name: string, newName: string): Observable<Categories> {
    const url = `${this.connectionUrl}categories/${name}`;
    return this.http
      .put<Categories>(url, { name: newName })
      .pipe(catchError(() => of({} as Categories)));
  }
  ...
  getAllTypes(): Observable<Types[]> {
    ...
    return this.http.get<Types[]>(url).pipe(catchError(() => of([])));
  }

  getTypeByName(name: string): Observable<Types> {
    const url = `${this.connectionUrl}types/${name}`;
    return this.http.get<Types>(url).pipe(catchError(() => of({} as Types)));
  }

  createType(name: string): Observable<Types> {
    const url = `${this.connectionUrl}types`;
    return this.http
      .post<Types>(url, { name: name })
      .pipe(catchError(() => of({} as Types)));
  }

  updateTypeByName(name: string, newName: string): Observable<Types> {
    const url = `${this.connectionUrl}types/${name}`;
    return this.http
      .put<Types>(url, { name: newName })
      .pipe(catchError(() => of({} as Types)));
  }

  deleteTypeByName(name: string): Observable<Types> {
    const url = `${this.connectionUrl}types/${name}`;
    return this.http.delete<Types>(url).pipe(catchError(() => of({} as Types)));
  }
  ...
  getAllPrivileges(): Observable<Privileges[]> {
    ...
    return this.http.get<Privileges[]>(url).pipe(catchError(() => of([])));
  }
  ...
  getPrivilegeByName(name: string): Observable<Privileges> {
    ...
    return this.http
      .get<Privileges>(url)
      .pipe(catchError(() => of({} as Privileges)));
  }

  createPrivilege(name: string): Observable<Privileges> {
    const url = `${this.connectionUrl}privileges`;
    return this.http
      .post<Privileges>(url, { name: name })
      .pipe(catchError(() => of({} as Privileges)));
  }
  ...
  updatePrivilegeByName(name: string, newName: string): Observable<Privileges> {
    const url = `${this.connectionUrl}privileges/${name}`;
    return this.http
      .put<Privileges>(url, { name: newName })
      .pipe(catchError(() => of({} as Privileges)));
  }
  ...
  getAllUsers(): Observable<Users[]> {
    ...
    return this.http.get<Users[]>(url).pipe(catchError(() => of([])));
  }

  getUserByEmail(email: string): Observable<Users> {
    const url = `${this.connectionUrl}users/${email}`;
    return this.http.get<Users>(url).pipe(catchError(() => of({} as Users)));
  }
  ...
  createUser(email: string, privileges: number[]): Observable<Users> {
    ...
    const url = `${this.connectionUrl}users`;
    return this.http
      .post<Users>(url, { email, password, privileges })
      .pipe(catchError(() => of({} as Users)));
  }
  ...
  updateUserByEmail(
    ...
    const url = `${this.connectionUrl}users/${email}`;
    return this.http
      .put<DBResponse>(url, { newEmail, password, privileges })
      .pipe(catchError(() => of({} as DBResponse)));
  }

  deleteUserByEmail(email: string): Observable<DBResponse> {
    const url = `${this.connectionUrl}users/${email}`;
    return this.http
      .delete<DBResponse>(url)
      .pipe(catchError(() => of({} as DBResponse)));
  }
  ...
  forgotPassword(email: string, privileges: number[]): void {
    ...
    // actualiza en la bd
    this.updateUserByEmail(email, email, password, privileges).subscribe(
      (response) => {}
    );
  }
  ...
  getAllServices(): Observable<Services[]> {
    ...
    return this.http.get<Services[]>(url).pipe(catchError(() => of([])));
  }
  ...
  getServices(): void {
    ...
    this.http
      .get<{ services: Services[]; totalCount: number }>(url, { params })
      .pipe(
  ...
  getServiceByName(name: string): Observable<Services> {
    ...
    return this.http
      .get<Services>(url)
      .pipe(catchError(() => of({} as Services)));
  }
  ...
  createService(
    ...
    const url = `${this.connectionUrl}services`;
    return this.http
      .post<Services>(url, { name, description, price, images })
      .pipe(catchError(() => of({} as Services)));
  }
  ...
  updateServiceByName(
    ...
    const url = `${this.connectionUrl}services/${name}`;
    return this.http
      .put<Services>(url, { name: newName, description, price, images })
      .pipe(catchError(() => of({} as Services)));
  }
  ...
  getAllProducts(): Observable<Products[]> {
    ...
    return this.http.get<Products[]>(url).pipe(catchError(() => of([])));
  }
  ...
  getProducts(): void {
    ...
    this.http
      .get<{ products: Products[]; totalCount: number }>(url, { params })
      .pipe(
  ...
  filterProducts(
    ...
    this.http
      .get<{ products: Products[]; totalCount: number }>(url)
      .pipe(
        catchError(() => of({ products: [], totalCount: 0 })),
        tap((response) => {
          // Actualizar los atributos del servicio con la respuesta del servidor
  ...
  getData(): Observable<Data[]> {
    ...
    return this.http.get<Data[]>(url).pipe(catchError(() => of([])));
  }
  ...
  updateMainPage(
    ...
    return this.http
      .put<Data>(url, { slogan, description, mision, vision, logo })
      .pipe(
  ...
  updateMisionImages(visionImages: string[]): Observable<Data> {
    ...
    return this.http.put<Data>(url, { visionImages }).pipe(
      catchError((error: HttpErrorResponse) => {
        // Aquí puedes devolver un mensaje de error específico
        return throwError('Hubo un error al procesar la solicitud');
      })
  ...
  updatePresentationImages(presentationImages: string[]): Observable<Data> {
    ...
    return this.http.put<Data>(url, { presentationImages }).pipe(
      catchError((error: HttpErrorResponse) => {
        // Aquí puedes devolver un mensaje de error específico
        return throwError('Hubo un error al procesar la solicitud');
      })
  ...
  getContactData(): Observable<Contact[]> {
    ...
    return this.http.get<Contact[]>(url).pipe(catchError(() => of([])));
  }
